...
This package provides the function `process_forecasts` that takes as input the training dataset with the time series and a list of forecasting methods to apply to it. We can see in the documentation of the function `process_forecasts` that the input dataset requires the following format: A `list` with each element having the following structure:
...
````{r eval=TRUE}
#this will take time
M4_train <- process_forecasts(M4_train, forec_methods(), n.cores=2)
#once we have the forecasts, we can calculate the errors
M4_train <- process_errors(M4_train)
```

Now we have in `M4_train` the series, the forecasts of each method for each series, and the errors of these forecasts. Specifically, `process_forecasts` produces as output a list, with each element having the following structure:
...
The specific features extracted from the series are a key part of the process, and in this package, the function `process_THA_features` calculates such features. A rationale and description of the features may be seen in a --upcoming paper from (Talagala, Hyndman and Athanasopoulos, 2018)--, though the actual set of features extracted is slightly different.

`process_THA_features` processes once again a dataset of times series, a list of elements containing at least the field `x` with a `ts` object (in the spirit of the input and output datasets of `process_forecasts` and the `Mcomp` package data format).


The output of `process_THA_features` is its input list, but to each element, the field `features` has been added. `features` is a `tibble` object with the extracted features. `process_THA_features` uses the package `tsfeatures` for extracting the features.
...
````{r}
M4_train <- process_THA_features(M4_train)
````
...
````{r}
M4_test <- process_forecasts(M4_test, forec_methods(), n.cores=2)
M4_test <- process_THA_features(M4_test, n.cores=2)
````
...
will take the `predictions` and the `dataset` in the output of format of `process_forecasts` with the `ff` field and add the field `y_hat` with the actual forecasts
