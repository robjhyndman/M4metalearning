...
````{r}
tcf <- randomForest::randomForest(train_data$data, as.factor(train_data$labels))
forest_preds <- predict(tcf, newdata=test_data$data, type="prob")
summary_performance(forest_preds, dataset = M4metalearning::feat_forec_M3)
````

We see that the randomForest model produces very similar results to the training set

The results on the training

How well do they extrapolate?

````{r}

test_data <- create_feat_classif_problem(M4metalearning::feat_forec_M1)
meta_preds <- predict_selection_ensemble(tempcross_model, test_data$data)
summary_performance(meta_preds, dataset = M4metalearning::feat_forec_M1)

forest_preds <- predict(tcf, newdata=test_data$data, type="prob")
summary_performance(forest_preds, dataset = M4metalearning::feat_forec_M1)
````
