...
'Oracle OWI' shows the theoretical minimum error that a classifier would produce, 'Single method OWI' is the best method in our pool of forecasting methods, and 'Average OWI' would be the error produced by selecting methods at random from our pool of methods for each series. 'Naive weighted' is the error produced by averaging the forecasts of the methods to produce a new forecast.
...
The 'Naive Weighted OWI' is also better than the single best method.
